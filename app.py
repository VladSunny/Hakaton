...
email_domen = "gmail.com" #"students.sch2.ru"

app.config['MAIL_SERVER'] = 'smtp.gmail.com'  # SMTP сервер
app.config['MAIL_PORT'] = 587
app.config['MAIL_USE_TLS'] = True
app.config['MAIL_USE_SSL'] = False
app.config['MAIL_USERNAME'] = 'predprof.hackaton@gmail.com'  # <-- ВАША ПОЧТА
app.config['MAIL_PASSWORD'] = 'vxha cffy tcqd czug'     # <-- ПАРОЛЬ ПРИЛОЖЕНИЯ
app.config['MAIL_DEFAULT_SENDER'] = 'predprof.hackaton@gmail.com'  # <-- ВАША ПОЧТА
...
def send_email(to_email, subject, code, template_type='verify'):
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px;">
        <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px; border-radius: 15px 15px 0 0;">
            <h1 style="color: white; margin: 0; text-align: center;"> Школьное Питание </h1>
        </div>
        ...
        <div style="background: #f9fafb; padding: 20px; border-radius: 0 0 15px 15px; text-align: center; border: 1px solid #e5e7eb; border-top: none;">
            <p style="color: #9ca3af; margin: 0; font-size: 12px;">© 2026 Школьное Питание. Все права защищены.</p>
        </div>
    </div>
    '''
...
def verify():
    ...
    name = session['name']
    hash_password = session['hash_password']
    ...
    if request.method == 'POST':
        ...
        notification = Notification(user_id=user.id, text='Добро пожаловать в систему школьного питания!')
...
def register():
    ...
    if request.method == 'POST':
        ...
        if email[email.find("@") + 1:] != email_domen:
            flash('Неразрешенный домен адреса', 'error')
            return redirect(url_for('register'))
        ...
        success, error = send_email(email, 'Подтверждение регистрации', code, 'register')
        
        if success:
            session['name'] = name
            session['hash_password'] = generate_password_hash(password)
            session['hash_password'] = generate_password_hash(password)
...
def student_menu():
    ...
    return render_template('student/menu.html', breakfast=breakfast, lunch=lunch, user_allergies=user_allergies)
...
def student_profile():
    ...
    return render_template('student/profile.html', all_allergens=all_allergens, user_allergies=user_allergies, orders_count=orders_count)
...
def checkout():
    ...
    return jsonify({'success': True, 'message': 'Заказ успешно оформлен!', 'new_balance': current_user.balance})
...
def topup():
    ...
    return jsonify({'success': True, 'message': f'Баланс пополнен на {amount} ₽', 'new_balance': current_user.balance})
...
def buy_subscription():
    ...
    subscription = Subscription(
        user_id=current_user.id,
        sub_type=sub_type,
        remaining_meals=meals[sub_type]
    )
...
def cook_serve():
    ...
    return render_template('cook/serve.html', served=served, pending_orders=pending_orders, now=now)
...
def cook_inventory():
    ...
    return render_template('cook/inventory.html', products=products, low_count=low_count)
...
def serve_meal(meal_type):
    ...
    db.session.commit()
    return jsonify({'success': True, 'breakfast': served.breakfast_count, 'lunch': served.lunch_count})
...
def admin_dashboard():
    ...
    if yesterday_orders > 0:
        orders_change = int(((today_orders - yesterday_orders) / yesterday_orders) * 100)
    ...
    ).group_by(OrderItem.name).order_by(func.count(OrderItem.id).desc()).limit(5).all()
...
def admin_reports():
    ...
    breakfast_revenue = db.session.query(func.sum(Order.total)).filter_by(meal_type='breakfast').scalar() or 0
    lunch_revenue = db.session.query(func.sum(Order.total)).filter_by(meal_type='lunch').scalar() or 0
    
    # Average prices
    avg_breakfast_price = breakfast_revenue / breakfast_count if breakfast_count > 0 else 0
...
def init_db():
    with app.app_context():
        ...
        if User.query.first() is None:
            # Create test users
            users = [
                User(email='student@school.ru', password_hash=generate_password_hash('123456'), 
                     name='Петров Алексей', role='student', student_class='9А', balance=1500),
                User(email='cook@school.ru', password_hash=generate_password_hash('123456'),
                     name='Сидорова Мария', role='cook'),
                User(email='admin@school.ru', password_hash=generate_password_hash('123456'),
                     name='Иванов Сергей', role='admin')
            ]
            db.session.add_all(users)
            
            # Create menu items
            menu_items = [
                MenuItem(name='Каша овсяная с фруктами', meal_type='breakfast', price=80, calories=250, allergens='глютен,молоко', image='🥣'),
                MenuItem(name='Омлет с сыром', meal_type='breakfast', price=95, calories=320, allergens='яйца,молоко', image='🍳'),
                MenuItem(name='Блинчики с творогом', meal_type='breakfast', price=110, calories=380, allergens='глютен,молоко,яйца', image='🥞'),
                MenuItem(name='Йогурт с мюсли', meal_type='breakfast', price=75, calories=200, allergens='молоко,глютен', image='🥛'),
                MenuItem(name='Борщ украинский', meal_type='lunch', price=120, calories=280, allergens='', image='🍲'),
                MenuItem(name='Котлета куриная с пюре', meal_type='lunch', price=150, calories=450, allergens='глютен,яйца', image='🍖'),
                MenuItem(name='Рыба запеченная с овощами', meal_type='lunch', price=180, calories=380, allergens='рыба', image='🐟'),
                MenuItem(name='Макароны с сыром', meal_type='lunch', price=100, calories=420, allergens='глютен,молоко', image='🍝'),
                MenuItem(name='Салат овощной', meal_type='lunch', price=70, calories=120, allergens='', image='🥗'),
                MenuItem(name='Компот из сухофруктов', meal_type='lunch', price=30, calories=80, allergens='', image='🍹'),
            ]
